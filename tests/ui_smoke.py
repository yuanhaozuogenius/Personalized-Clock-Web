...
def main():
    ...
    assert cdp.evaluate("[...document.querySelector('.action-row').children].slice(-3).map(item=>item.textContent.trim()).join('|') === '使用说明|更新日志|分享'")
    ...
    cdp.evaluate("const input=document.querySelector('#background-opacity'); input.value='100'; input.dispatchEvent(new Event('input',{bubbles:true})); document.querySelector('#background-fit-trigger').click();")
    wait_for(cdp, "document.querySelector('#background-fit-menu').hidden === false")
    time.sleep(0.25)
    assert cdp.evaluate("(() => { const trigger=getComputedStyle(document.querySelector('#background-fit-trigger')); const options=[...document.querySelectorAll('.wallpaper-fit-option')].map(getComputedStyle); return trigger.alignItems==='center' && trigger.justifyContent==='center' && options.every(style=>style.alignItems==='center'&&style.justifyContent==='center'&&style.textAlign==='center'&&style.fontStyle==='normal'); })()")
    assert cdp.evaluate("getComputedStyle(document.querySelector('.theme-chevron')).transform.startsWith('matrix(-1')")
    menu_screenshot = cdp.call("Page.captureScreenshot", {"format": "png", "captureBeyondViewport": False})
    screenshot_path = Path(args.screenshot)
    menu_path = screenshot_path.with_name(f"{screenshot_path.stem}-wallpaper-menu{screenshot_path.suffix}")
    menu_path.write_bytes(base64.b64decode(menu_screenshot["data"]))
    cdp.evaluate("document.querySelector('.wallpaper-fit-option[data-fit=contain]').click()")
    ...
    assert cdp.evaluate("document.querySelector('#background-fit-label').textContent === '完整显示' && document.querySelector('#background-fit-menu').hidden")
    ...
    cdp.evaluate("document.querySelector('.action-row a[href=\"changelog.html\"]').click()")
    wait_for(cdp, "document.readyState === 'complete' && location.pathname.endsWith('/changelog.html') && document.querySelector('h1')?.textContent.includes('更新日志')")
    assert cdp.evaluate("document.body.textContent.includes('V17') && document.body.textContent.includes('V16')")
    assert cdp.evaluate("(() => { const back=document.querySelector('.guide-back').getBoundingClientRect(); const icon=document.querySelector('.guide-back-icon').getBoundingClientRect(); return Math.abs((back.top+back.height/2)-(icon.top+icon.height/2))<1; })()")
    release_screenshot = cdp.call("Page.captureScreenshot", {"format": "png", "captureBeyondViewport": False})
    release_path = screenshot_path.with_name(f"{screenshot_path.stem}-changelog{screenshot_path.suffix}")
    release_path.write_bytes(base64.b64decode(release_screenshot["data"]))
    cdp.evaluate("document.querySelector('.guide-back').click()")
    wait_for(cdp, "document.readyState === 'complete' && location.pathname.endsWith('/index.html')")
    ...
    assert cdp.evaluate("(() => { const back=document.querySelector('.guide-back').getBoundingClientRect(); const icon=document.querySelector('.guide-back-icon').getBoundingClientRect(); return Math.abs((back.top+back.height/2)-(icon.top+icon.height/2))<1; })()")
    help_screenshot = cdp.call("Page.captureScreenshot", {"format": "png", "captureBeyondViewport": False})
    help_path = screenshot_path.with_name(f"{screenshot_path.stem}-help{screenshot_path.suffix}")
    help_path.write_bytes(base64.b64decode(help_screenshot["data"]))
